...
def cmd_check(args):
    ...
    verify_script = (
        Path(__file__).parent / "components" / "py-sstv-groundstation" / "verify_rtlsdr.py"
    )
...
Примеры использования:
  ...
  %(prog)s auto-record --lat 55.75 --lon 37.61  # Автозапись
    ...
    passes_parser.add_argument(
        "--hours", "-t", type=int, default=24, help="На сколько часов вперёд"
    )
    ...
    rec_parser.add_argument(
        "--duration", "-d", type=int, default=60, help="Длительность записи (сек)"
    )
    ...
    auto_parser.add_argument(
        "--interval", "-i", type=int, default=30, help="Интервал проверки (сек)"
    )
