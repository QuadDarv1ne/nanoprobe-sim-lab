...
DATETIME_IMPORT_PATTERN = re.compile(r"from\s+datetime\s+import\s+([^#\n]+)")
...
def migrate_file(filepath: Path, dry_run: bool = False) -> dict:
    ...
    stats = {"file": str(filepath), "changes": 0, "added_import": False, "backup": None}
    ...
    backup_path = filepath.with_suffix(
        f'.py.bak.{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")}'
    )
    try:
    ...
    try:
        ...
        logger.info(
            f"✓ {filepath}: {stats['changes']} замен, import добавлен: {stats['added_import']}"
        )
    except Exception as e:
