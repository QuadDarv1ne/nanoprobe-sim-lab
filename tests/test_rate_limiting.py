...
def test_ip_whitelist():
    ...
    # Добавление
    asyncio.run(whitelist_ip("192.168.1.1"))
    asyncio.run(whitelist_ip("10.0.0.1"))

    assert asyncio.run(is_ip_whitelisted("192.168.1.1")), "IP должен быть в whitelist"
    assert asyncio.run(is_ip_whitelisted("10.0.0.1")), "IP должен быть в whitelist"
    assert not asyncio.run(is_ip_whitelisted("192.168.1.2")), "IP не должен быть в whitelist"

    # Удаление
    from api.rate_limiter import remove_from_whitelist

    asyncio.run(remove_from_whitelist("192.168.1.1"))
    assert not asyncio.run(is_ip_whitelisted("192.168.1.1")), "IP должен быть удалён из whitelist"
...
def test_ip_blacklist():
    ...
    # Добавление
    asyncio.run(blacklist_ip("192.168.1.100"))
    asyncio.run(blacklist_ip("10.0.0.100"))

    assert asyncio.run(is_ip_blacklisted("192.168.1.100")), "IP должен быть в blacklist"
    assert asyncio.run(is_ip_blacklisted("10.0.0.100")), "IP должен быть в blacklist"
    assert not asyncio.run(is_ip_blacklisted("192.168.1.101")), "IP не должен быть в blacklist"

    # Удаление
    from api.rate_limiter import remove_from_blacklist

    asyncio.run(remove_from_blacklist("192.168.1.100"))
    assert not asyncio.run(is_ip_blacklisted("192.168.1.100")), "IP должен быть удалён из blacklist"
...
def test_get_ip_lists():
    ...
    # Очистка
    from api.rate_limiter import _ip_blacklist, _ip_whitelist
    ...
    lists = asyncio.run(get_ip_lists())
