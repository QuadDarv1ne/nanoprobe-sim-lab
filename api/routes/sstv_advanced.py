...
class SSTVSessionManager:
    ...
    def get_stats(self) -> dict:
        """Получить статистику"""
        return {
            'active_recordings': len([s for s in self.recording_sessions.values() if s['status'] == 'recording']),
...
async def get_sstv_status():
    ...
    return {
        ...
        'modes': list(RECEIVER_AVAILABLE and __import__('api.sstv.rtl_sstv_receiver', fromlist=['SSTV_MODES']).SSTV_MODES.keys()) if RECEIVER_AVAILABLE else [],
    }
