...
class CodeAnalyzer:
        ...
        if lines_of_code > 0:
            ...
            maintainability_index = max(
                ...
                min(
                    100,
                    171
                    - 5.2 * (complexity / lines_of_code * 100)
                    - 0.23 * (lines_of_code)
                    - 16.2 * (comment_ratio),
                ),
            )
    ...
    def _generate_analysis_report(self) -> Dict[str, Any]:
        ...
        avg_complexity = (
            sum(m.complexity for m in self.metrics) / total_files if total_files > 0 else 0
        )
        avg_maintainability = (
            sum(m.maintainability_index for m in self.metrics) / total_files
            if total_files > 0
            else 0
        )
