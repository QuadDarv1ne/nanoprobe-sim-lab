...
class ScientificPDFReport:
    ...
    def _create_title_page(self, title: str, author: str, organization: str) -> List:
    ...
    def _add_images(self, images: List[str]) -> List:
        ...
        for i, img_path in enumerate(images, 1):
            try:
                ...
                content.append(img)
                content.append(
                    Paragraph(f"<i>Рисунок {i}: {Path(img_path).stem}</i>", self.styles["BodyText"])
                )
                content.append(Spacer(1, 0.3 * inch))
    ...
    def _create_conclusions(self, data: Dict[str, Any]) -> List:
        ...
        if "mean_height" in data:
            ...
            if abs(mean) < 10:
                conclusions.append(
                    "• Поверхность имеет низкий рельеф с средней высотой менее 10 нм"
                )
            elif abs(mean) < 100:
    ...
    def generate_defect_analysis_report(
        ...
        if defects:
            ...
            defect_table = Table(
                defect_table_data, colWidths=[0.8 * cm, 3 * cm, 3.5 * cm, 2.5 * cm, 2.5 * cm]
            )
        ...
        if defects:
            ...
            content.append(
                Paragraph(
                    "• Поверхность соответствует требованиям качества", self.styles["BodyText"]
                )
            )
    ...
    def generate_batch_report(
        self, batch_data: Dict[str, Any], title: str = "Отчёт о пакетной обработке"
    ) -> str:
        ...
        content = []
        content.extend(
            self._create_title_page(title, "Nanoprobe Simulation Lab", "Batch Processing System")
        )
        content.append(PageBreak())
    ...
    def _create_comparison_table(self, data: Dict[str, Any]) -> List:
        ...
        metrics = [
            (
                "SSIM",
                f"{data.get('ssim', 0):.4f}",
                "Высокое сходство" if data.get("ssim", 0) > 0.8 else "Различия",
            ),
            (
                "PSNR (dB)",
                f"{data.get('psnr', 0):.2f}",
                "Высокое" if data.get("psnr", 0) > 30 else "Низкое",
            ),
            (
                "Similarity",
                f"{data.get('similarity', 0):.4f}",
                "Похожи" if data.get("similarity", 0) > 0.7 else "Различаются",
            ),
            (
                "MSE",
                f"{data.get('mse', 0):.6f}",
                "Малая ошибка" if data.get("mse", 0) < 0.01 else "Заметная ошибка",
            ),
            (
                "Pearson",
                f"{data.get('pearson', 0):.4f}",
                "Коррелируют" if abs(data.get("pearson", 0)) > 0.7 else "Слабая корреляция",
            ),
        ]
    ...
    def _create_difference_analysis(self, data: Dict[str, Any]) -> List:
        """Анализ различий"""
        content = []
        mean_diff, max_diff, std_diff = (
            data.get("mean_diff", 0),
            data.get("max_diff", 0),
            data.get("std_diff", 0),
        )
        content.append(
            Paragraph(f"<b>Средняя разница:</b> {mean_diff:.4f}", self.styles["BodyText"])
        )
        content.append(Spacer(1, 0.1 * inch))
        content.append(
            Paragraph(f"<b>Максимальная разница:</b> {max_diff:.4f}", self.styles["BodyText"])
        )
        content.append(Spacer(1, 0.1 * inch))
        content.append(
            Paragraph(f"<b>Стандартное отклонение:</b> {std_diff:.4f}", self.styles["BodyText"])
        )
        content.append(Spacer(1, 0.2 * inch))
        if mean_diff < 0.05:
            content.append(
                Paragraph(
                    "<b>Оценка:</b> Поверхности практически идентичны", self.styles["BodyText"]
                )
            )
        elif mean_diff < 0.15:
            content.append(
                Paragraph(
                    "<b>Оценка:</b> Поверхности имеют небольшие различия", self.styles["BodyText"]
                )
            )
        else:
            content.append(
                Paragraph(
                    "<b>Оценка:</b> Поверхности существенно различаются", self.styles["BodyText"]
                )
            )
        return content
    ...
    def _create_comparison_conclusions(self, data: Dict[str, Any]) -> List:
        ...
        if similarity > 0.8 and ssim > 0.8:
            content.append(
                Paragraph("• Поверхности имеют высокую степень сходства", self.styles["BodyText"])
            )
            content.append(
                Paragraph(
                    "• Могут считаться эквивалентными для большинства задач",
                    self.styles["BodyText"],
                )
            )
        elif similarity > 0.5:
            content.append(
                Paragraph("• Поверхности имеют умеренное сходство", self.styles["BodyText"])
            )
            content.append(
                Paragraph(
                    "• Требуется дополнительный анализ для критичных применений",
                    self.styles["BodyText"],
                )
            )
        else:
            content.append(
                Paragraph("• Поверхности существенно различаются", self.styles["BodyText"])
            )
            content.append(
                Paragraph(
                    "• Не рекомендуются для взаимозаменяемого использования",
                    self.styles["BodyText"],
                )
            )
        return content
    ...
    def _create_simulation_results(self, data: Dict[str, Any]) -> List:
        ...
        content.append(
            Paragraph(
                data.get("results_summary", "Симуляция завершена успешно"), self.styles["BodyText"]
            )
        )
        return content
    ...
    def _create_batch_statistics(self, data: Dict[str, Any]) -> List:
        ...
        content = []
        success_rate = (
            (data.get("success_count", 0) / data.get("total_items", 1)) * 100
            if data.get("total_items", 0) > 0
            else 0
        )
        content.append(
            Paragraph(f"<b>Процент успеха:</b> {success_rate:.1f}%", self.styles["BodyText"])
        )
