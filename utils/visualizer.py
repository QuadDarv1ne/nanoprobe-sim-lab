...
class SurfaceVisualizer:
    ...
    def animate_scan_process(
        self,
        scan_data_list: list,
        title: str = "Процесс сканирования",
        save_path: Optional[str] = None,
        ...
        ani = animation.FuncAnimation(
            fig, update, frames=len(scan_data_list), interval=200, blit=True, repeat=True
        )
...
class SSTVVisualizer:
    ...
    def plot_decoded_image(
        self,
        image_data,
        title: str = "Декодированное SSTV изображение",
        save_path: Optional[str] = None,
    ) -> plt.Figure:
        """
        ...
        try:
            ...
            if surface_data is not None:
                fig1 = self.surface_viz.plot_surface_2d(
                    surface_data, "Результаты моделирования СЗМ", output_path / "spm_surface_2d.png"
                )
                plt.close(fig1)

                fig2 = self.surface_viz.plot_surface_3d(
                    surface_data, "3D модель поверхности", output_path / "spm_surface_3d.png"
                )
                plt.close(fig2)
            ...
            if sstv_image is not None:
                fig5 = self.sstv_viz.plot_decoded_image(
                    sstv_image, "Декодированное SSTV изображение", output_path / "sstv_decoded.png"
                )
                plt.close(fig5)
...
def main():
    ...
    logger.info("=== ВИЗУАЛИЗАТОР ПРОЕКТА ===")
    ...
    if success:
        ...
        logger.error("✗ Ошибка создания визуализаций")
