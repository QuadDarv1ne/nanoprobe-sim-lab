...
class MonitoringDashboard:
    ...
    def create_gui(self):
        ...
        ttk.Label(metrics_frame, text="Здоровье системы:").grid(
        ...
        ttk.Label(metrics_frame, text="Статус:").grid(
            row=1, column=2, sticky=tk.W, padx=(20, 0), pady=(10, 0)
        )
        ...
        self.fig, ((self.ax_cpu, self.ax_memory), (self.ax_disk, self.ax_network)) = plt.subplots(
            2, 2, figsize=(12, 8)
        )
        ...
        self.start_button = ttk.Button(
            control_frame, text="Запустить", command=self.start_monitoring
        )
        self.start_button.pack(side=tk.LEFT, padx=5)

        self.stop_button = ttk.Button(
            control_frame, text="Остановить", command=self.stop_monitoring
        )
        self.stop_button.pack(side=tk.LEFT, padx=5)
        self.stop_button.config(state=tk.DISABLED)

        self.refresh_button = ttk.Button(
            control_frame, text="Обновить", command=self.update_display
        )
        self.refresh_button.pack(side=tk.LEFT, padx=5)

        self.report_button = ttk.Button(
            control_frame, text="Создать отчет", command=self.generate_report
        )
    ...
    def animate_plots(self, frame):
        ...
        if trends["timestamps"]:
            ...
            if len(trends["network_sent"]) > 1:
                ...
                sent_rates = [
                    trends["network_sent"][i + 1] - trends["network_sent"][i]
                    for i in range(len(trends["network_sent"]) - 1)
                ]
                recv_rates = [
                    trends["network_recv"][i + 1] - trends["network_recv"][i]
                    for i in range(len(trends["network_recv"]) - 1)
                ]

                self.ax_network.plot(
                    network_times[:-1], sent_rates, "c-", label="Отправлено", linewidth=1
                )
                self.ax_network.plot(
                    network_times[:-1], recv_rates, "m-", label="Получено", linewidth=1
                )
...
class HealthCheckManager:
    ...
    def _check_project_configuration(self) -> Dict[str, Any]:
        ...
        return {"healthy": len(issues) == 0, "files_checked": len(config_files), "issues": issues}
...
def main():
    ...
    print(
        f"✓ Здоровье системы: {system_health['health_level']} ({system_health['health_score']}/100)"
    )
