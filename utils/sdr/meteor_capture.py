"""
Meteor-M N2 LRPT Auto-capture Manager

Schedules and automated Meteor-M N2 LRPT image captures via rtl_fm
at ~137.1 MHz. Uses satellite tracker for pass predictions.
"""
...
class MeteorCaptureManager:
    """Manages Meteor-M N2 LRPT auto-capture using rtl_fm SDR receiver.

    Tracks Meteor satellite passes and automatically captures
    LRPT transmissions during scheduled overpasses.
    """

    METEOR_FREQ = 137.1  # MHz, Meteor-M N2 LRPT

    def __init__(
        self,
        satellite_tracker=None,
        output_dir: str = "output/sstv/meteor/",
    ):
        ...
        self._state: Dict[str, Any] = {
            "capturing": False,
            "satellite": None,
            "frequency_mhz": self.METEOR_FREQ,
            "start_time": None,
        ...
        logger.info("MeteorCaptureManager initialized (output_dir=%s)", self._output_dir)

    # ── Public API ───────────────────────────────────────────────────────

    def schedule_auto_capture(self, satellite_name: str, pass_info: Dict[str, Any]) -> bool:
        """Schedule a capture for an upcoming Meteor satellite pass.

        Args:
            satellite_name: e.g. 'meteor_m2_3'
        ...
        logger.info(
            "Meteor capture scheduled: %s at %s (elev=%.1f)",
            satellite_name,
            aos_str,
    ...
    def start_capture(
        ...
        frequency: float,
        duration: int,
        satellite_name: str = "meteor",
    ) -> bool:
        """Start Meteor LRPT capture using rtl_fm subprocess.

        Args:
            frequency: Frequency in MHz (default 137.1).
        ...
        logger.info(
            "Starting Meteor LRPT capture: %s at %.3f MHz, duration=%ds",
            satellite_name,
            frequency,
            duration,
        )
    ...
    def stop_capture(self) -> Optional[str]:
        ...
        with self._lock:
            ...
            logger.info("Stopping Meteor LRPT capture")
            try:
                proc.terminate()
    ...
    def start_auto_scheduler(self, check_interval: int = 60):
        ...
        self._scheduler_thread = threading.Thread(
            ...
            args=(check_interval,),
            daemon=True,
            name="Meteor-auto-scheduler",
        )
        self._scheduler_thread.start()
        logger.info("Meteor auto-scheduler started (interval=%ds)", check_interval)
    ...
    def stop_auto_scheduler(self):
        ...
        if self._scheduler_thread:
            ...
            logger.info("Meteor auto-scheduler stopped")

    # ── Internal ─────────────────────────────────────────────────────────

    def _auto_schedule(self, check_interval: int):
        """Background loop: checks for upcoming Meteor passes and auto-captures."""
        meteor_sats = ["meteor_m2_3"]

        while self._scheduler_running:
            try:
                for sat_name in meteor_sats:
                    ...
                    if 0 < time_to_aos < 300:
                        ...
                        self.start_capture(self.METEOR_FREQ, int(duration), sat_name)
                        break

            except Exception:
                logger.exception("Error in Meteor auto-scheduler loop")
