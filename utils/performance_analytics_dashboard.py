...
class PerformanceAnalyticsDashboard:
    ...
    def _calculate_avg_execution_time(self) -> float:
        ...
        execution_times = [
            metric.value
            for metric in self.performance_profiler.metrics
    ...
    def _calculate_memory_efficiency(self) -> float:
        ...
        memory_usages = [
            metric.value for metric in self.performance_profiler.metrics if "memory" in metric.name
        ]
    ...
    def _analyze_trends(self):
        ...
        for data in self.performance_history:
            metrics_over_time["efficiency"].append(
                data["performance_metrics"].get("efficiency_score", 0)
            )
            metrics_over_time["cpu_percent"].append(data["resource_metrics"].get("cpu_percent", 0))
            metrics_over_time["memory_percent"].append(
                data["resource_metrics"].get("memory_percent", 0)
            )
            metrics_over_time["rss_mb"].append(data["memory_metrics"].get("rss_mb", 0))
            metrics_over_time["execution_time"].append(
                data["performance_metrics"].get("avg_execution_time", 0)
            )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        efficiency_values = [
            d["performance_metrics"].get("efficiency_score", 0) for d in self.performance_history
        ]
        cpu_values = [d["resource_metrics"].get("cpu_percent", 0) for d in self.performance_history]
        memory_values = [
            d["resource_metrics"].get("memory_percent", 0) for d in self.performance_history
        ]
    ...
    def get_actionable_insights(self, severity_filter: str = "high") -> List[PerformanceInsight]:
        ...
        filtered_insights = [
            ...
            or insight.severity == severity_filter
    ...
    def generate_analytics_report(self, output_path: str = None) -> str:
        ...
        report = {
            ...
            "top_insights": [
                self._insight_to_dict(insight) for insight in self.get_actionable_insights("high")
            ],
    ...
    def _generate_comprehensive_recommendations(self) -> List[str]:
        ...
        if current.get("cpu_percent", 0) > 80:
            recommendations.append(
                "Оптимизируйте алгоритмы или распределите нагрузку для снижения загрузки CPU"
            )
        ...
        if self.trend_analysis and "trends" in self.trend_analysis:
            ...
            if "cpu_percent" in trends and trends["cpu_percent"]["direction"] == "increasing":
                recommendations.append(
                    "Загрузка CPU имеет положительный тренд - требуется оптимизация"
                )
        ...
        if not recommendations:
            recommendations.append(
                "Система работает в оптимальном режиме. Продолжайте текущую стратегию мониторинга."
            )
    ...
    def visualize_performance_dashboard(self, output_path: str = None) -> str:
        ...
        cpu_percents = [
            data["resource_metrics"].get("cpu_percent", 0) for data in self.performance_history
        ]
        memory_percents = [
            data["resource_metrics"].get("memory_percent", 0) for data in self.performance_history
        ]
        rss_mb_values = [
            data["memory_metrics"].get("rss_mb", 0) for data in self.performance_history
        ]

        # Создаем фигуру с подграфиками
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Аналитическая панель производительности - Nanoprobe Simulation Lab",
            fontsize=16,
            fontweight="bold",
        )
        ...
        axes[0, 0].axhline(
            y=80, color="orange", linestyle="--", alpha=0.7, label="Порог хорошей эффективности"
        )
        axes[0, 0].axhline(
            y=60, color="red", linestyle="--", alpha=0.7, label="Порог низкой эффективности"
        )
        ...
        axes[0, 1].axhline(
            y=80, color="orange", linestyle="--", alpha=0.7, label="Порог высокой загрузки"
        )
        axes[0, 1].axhline(
            y=95, color="red", linestyle="--", alpha=0.7, label="Порог критической загрузки"
        )
        ...
        axes[1, 0].plot(
            timestamps,
            [rss_mb_values[i] / 10 for i in range(len(rss_mb_values))] if rss_mb_values else [],
            linestyle=":",
            color="purple",
            label="RSS (деленная на 10)",
        )
        axes[1, 0].axhline(
            y=80, color="orange", linestyle="--", alpha=0.7, label="Порог высокого использования"
        )
